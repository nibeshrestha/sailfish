...
use crate::messages::{Certificate, Header, HeaderWithCertificate, HeaderInfoWithCertificate,NoVoteCert, NoVoteMsg, Timeout, TimeoutCert, Vote};
...
impl Core {
    ...
    async fn process_header_msg(
        ...
        self.tx_consensus_header_msg
            ...
            .await.expect("failed to send HeaderInfo to consensus");
    ...
    fn sanitize_vote(&mut self, vote: &Vote) -> DagResult<()> {
        if let Some(header) = self.processing_header_infos.get(&vote.id) {
            ensure!(
    ...
    pub async fn run(&mut self) {
        loop {
            ...
            let result = tokio::select! {
                ...
                Some(header_msg) = self.rx_header_waiter.recv() => self.process_header_msg(&header_msg, &sender_channel).await,
